...
def main():
    ...
    dataset = pd.read_csv("./../new_Base_CDM_balanced_V2.csv" , header =1) 
    

    #X, y = dataset['data'], dataset['target']
    X , y = dataset.iloc[:,[1,2,3,4,6]].values , dataset.iloc[:,0].values
    print(y)
    #feature_names, class_names = dataset['feature_names'], dataset['target_names']
    feature_names, class_names = dataset.columns[1:5] , dataset.columns[0]
